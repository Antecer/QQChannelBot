...
namespace QQChannelBot.Bot
{
    public partial class BotClient
    {
        ...
        {
            BotAPI api = APIList.获取频道成员列表;
            HttpResponseMessage? respone = await HttpSendAsync($"{api.Path.Replace("{guild_id}", guild_id)}?limit={limit}&after={after ?? "0"}", api.Method, null, sender);
        ...
        {
            BotAPI api = APIList.获取成员详情;
            HttpResponseMessage? respone = await HttpSendAsync(api.Path.Replace("{guild_id}", guild_id).Replace("{user_id}", user_id), api.Method, null, sender);
